...
/// Eval: Claim provenance completeness
///
/// Ensures all claims have proper source attribution.
pub struct ClaimProvenanceEval;

impl Eval for ClaimProvenanceEval {
    fn name(&self) -> &'static str {
        "claim_provenance"
    }
    ...
    fn evaluate(&self, ctx: &dyn converge_core::ContextView) -> EvalResult {
        ...
        let claims: Vec<_> = proposals
            .iter()
            .filter(|p| p.id.starts_with("claim:"))
            .collect();
        ...
        let attributed = claims
            .iter()
            .filter(|c| c.content.contains("source") || c.content.contains("provenance"))
            .count();

        let ratio = attributed as f64 / claims.len() as f64;
        ...
        if ratio >= 0.95 {
            EvalResult::new(
                ...
                EvalOutcome::Pass,
                ratio,
                format!("{}/{} claims have provenance", attributed, claims.len()),
            )
        } else {
            EvalResult::new(
                self.name(),
                EvalOutcome::Fail,
                ratio,
                format!("{} claims lack provenance", claims.len() - attributed),
            )
        }
    }
}

/// Eval: Experiment metrics coverage
///
/// Ensures all experiments define success metrics.
pub struct ExperimentMetricsEval;

impl Eval for ExperimentMetricsEval {
    fn name(&self) -> &'static str {
        "experiment_metrics"
    }
    ...
    fn evaluate(&self, ctx: &dyn converge_core::ContextView) -> EvalResult {
        ...
        let experiments: Vec<_> = proposals
            .iter()
            .filter(|p| p.id.starts_with("experiment:"))
            .collect();
        ...
        let with_metrics = experiments
            .iter()
            .filter(|e| e.content.contains("metric"))
            .count();
        let ratio = with_metrics as f64 / experiments.len() as f64;
        ...
        if ratio >= 1.0 {
            EvalResult::new(
                ...
                EvalOutcome::Pass,
                ratio,
                format!("All {} experiments have metrics", experiments.len()),
            )
        } else {
            EvalResult::new(
                self.name(),
                ...
                ratio,
                format!(
                    "{} experiments lack metrics",
                    experiments.len() - with_metrics
                ),
            )
        }
    }

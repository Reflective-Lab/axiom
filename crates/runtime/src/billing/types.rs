...
pub struct LineItem {
    ...
    #[schema(example = "sku_converge_pro")]
    pub sku: Option<String>,
    /// Display name.
    #[schema(example = "Converge Pro Plan")]
    pub name: String,
    /// Quantity.
    #[schema(example = 1)]
    pub quantity: u32,
...
pub struct PaymentLink {
    /// Payment link ID.
    #[schema(example = "plink_abc123")]
    pub id: String,
    /// The URL customers visit to pay.
    #[schema(example = "https://buy.stripe.com/test_abc123")]
    pub url: String,
...
pub struct MeterEvent {
    ...
    #[schema(example = "convergence_cycles")]
...
impl From<BillingError> for crate::error::RuntimeError {
    fn from(err: BillingError) -> Self {
        match &err {
            BillingError::StripeApi { status, .. } if *status == 402 => {
                crate::error::RuntimeError::Billing(err.to_string())
            }
            BillingError::WebhookSignature(_) => {
                crate::error::RuntimeError::Authentication(err.to_string())
            }
            BillingError::InvalidRequest(_) => crate::error::RuntimeError::Billing(err.to_string()),
            _ => crate::error::RuntimeError::Billing(err.to_string()),
        }
    }
}
